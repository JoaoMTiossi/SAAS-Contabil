...
export default function Sidebar({ mobileOpen, onClose }: SidebarProps) {
  ...
  const navItems = mainNavItems.filter((item) => modulos.includes(item.modulo));
  ...
  const sidebarContent = (
    <>
      ...
      <div className="flex h-16 items-center gap-3 border-b border-slate-700/50 px-6">
        <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-cyan-500 to-blue-600 text-sm font-bold text-white" style={{ fontFamily: "'Orbitron', sans-serif" }}>
          AH
        </div>
        <span className="text-base font-semibold text-white tracking-wider" style={{ fontFamily: "'Orbitron', sans-serif", letterSpacing: "0.15em" }}>APOLUM HORIZON</span>
      </div>
      ...
      <div className="border-t border-slate-700/50">
        ...
        <div className="border-t border-slate-700/50 px-6 py-3">
          <p className="truncate text-xs font-medium text-slate-400">{session.user.name}</p>
          <p className="truncate text-[11px] text-slate-600">{session.user.email}</p>
        </div>
      </div>
    </>
  );
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <aside className="hidden lg:flex lg:w-[260px] lg:flex-col lg:fixed lg:inset-y-0 bg-slate-900 z-40">
        {sidebarContent}
      </aside>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <aside className="fixed inset-y-0 left-0 flex w-[260px] flex-col bg-slate-900 shadow-2xl">
            {sidebarContent}
          </aside>
        </div>
      )}
